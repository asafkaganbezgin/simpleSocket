    ...
    def run(self):
        ...
        try:
            ...
            f = self.conn.makefile(
                buffering=1, encoding=ENCODING, newline=NEWLINE)
            ...
            while True:
                ...
                if cmd == "APND":
                    ...
                    try:
                        ...
                        send_response(self.conn, success=False, info=str(
                            version) + " is the current version, please get an update.")

                elif cmd == "WRTE":
                    dissected_args = args.split(" ", 2)
                    try:
                        if len(dissected_args) == 3 and int(dissected_args[0]) == version:
                            write(int(dissected_args[1]),
                                  dissected_args[2], file)
                            version += 1
                            send_response(self.conn, success=True,
                                          info=str(version))
                        else:
                        ...
                        send_response(self.conn, success=False,
                                      info="No such line exists.")

                elif cmd == "UPDT":
                    try:
                        if int(args) != version:
                            content = update(file)
                            send_updt_response(self.conn, success=True, info=str(
                        ...
                        send_response(self.conn, success=False, info=str(
                            version) + " is already the last version.")

                    except InvalidVersionException:
                        send_response(self.conn, success=False, info=str(
                            version) + " is invalid for Update.")
                    ...
                    send_response(self.conn, success=False, info=cmd +
                                  " command is already sent and processed.")
                    shutdown()
...
def auth_check(f, conn):
    ...
    if cmd == "USER":
        ...
        send_response(conn, success=False,
                      info="Wrong command. Expecting USER.")
    ...
    if cmd == "PASS":
        ...
        send_response(conn, success=False,
                      info="Wrong command. Expecting PASS.")
